...
export class OrdersController {
    constructor(private service: OrdersService, private usersService: UsersService) { }
    ...
    @Post('')
    @UseGuards(MainGuard)
    async placeOrder(@Req() req: MyRequest, @Body() data: OrderDto, @Res() res: Response) {
        const userId = req.user.userId
        const order = await this.service.create(userId, data);

        if (data.isSaved) {
            const response = await this.usersService.saveShippingAddress(userId, data.shippingAddress)
